...
| 4.1.2 | | | 시스템은 검색 결과(제목, 링크, 작성자)를 반환한다. | | | | - | H | |
| 4.1.3 | | | 사용자는 페이지를 통해 이슈 목록을 추가로 로드할 수 있다. | | | | - | H | |
| 4.2 | 기여도 측정 | 자신의 오픈소스 기여도를 확인한다. | | | | | | | |
| 4.2.1 | | | 시스템은 사용자의 커밋, PR, 이슈 생성 횟수를 한 번의 요청으로 조회한다. | | | | - | H | |
| 4.2.2 | | | 기여도 점수를 계산(이슈 5점, PR 3점, 커밋 1점). | | | | 5.3.1 | H | |
| 4.2.3 | | | 최신 기여 정보(DB)와 동기화한다. | | | | - | H | |
| 4.3 | 도전과제/배지 획득 | 기여 활동에 따라 보상을 획득한다. | | | | | | | |
| 4.3.1 | | | 기여도 점수로 등급 배지(브론즈~루비)를 수여한다. | | | | 5.2.2 | H | |
| 4.3.2 | | | 사용자는 과제 목록에서 목표 횟수와 현재 달성도를 확인할 수 있다. | | | | 5.2.3 | M | |
| 4.3.3 | | | 기여도 갱신 시 자동으로 완료 처리된다. | | | | 5.2.3 | M | |
...
| 4.4.2 | | | 등록된 Todo 목록을 최신순으로 무한 스크롤 조회한다. | | | | - | M | |
| 4.4.3 | | | Todo 완료 여부를 토글할 수 있다. | | | | - | M | |
| 4.4.4 | | | 여러 완료된 Todo를 선택하여 일괄 삭제할 수 있다. | | | | - | M | |
