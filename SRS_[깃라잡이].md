...
| 3.1.2 |  |  | 제목, 내용, 태그 입력 후 저장한다. | title="테스트 제목", content="테스트 내용", githubId | title(1~100), content≥1자, githubId | title="", content="", githubId 존재X | UC 3.1.2 | H | P |
| 3.1.3 |  |  | 저장 시 DB에 새로운 게시글 등록됨 | 저장 버튼 | postId 정상 생성 | DB 오류 | UC 3.1.3 | H | P |
| 3.2 | 게시글 수정 | 본인 글 수정 | 기존 게시글 내용을 수정한다. 작성자만 수정 가능하다. | '수정' 버튼 | 본인 게시글 | 타인 게시글 | UC 3.2 | H | P |
| 3.2.1 |  |  | 상세 화면에서 수정 버튼 클릭 → 수정 화면 진입 | 수정 버튼, postId, githubId | githubId(작성자) | githubId(작성자 아님) | UC 3.2.1 | H | P |
| 3.2.2 |  |  | 타인이 작성한 게시글에는 수정 버튼이 보이지 않는다. | postId, githubId | githubId(작성자 아님) |  | UC 3.2.2 | H | P |
| 3.2.3 |  |  | 수정 후 저장 시 DB의 게시글 데이터 갱신 | newTitle, newContent, githubId | newTitle(1~100), newContent≥1 | postId 없음, githubId 권한 없음 | UC 3.2.3 | H | P |
| 3.3 | 게시글 삭제 | Soft Delete | 작성자만 삭제 가능하며 논리적으로 삭제 처리된다. | 삭제 버튼, githubId | 본인 게시글 | githubId(작성자 아님) | UC 3.3 | H | P |
| 3.3.1 |  |  | 상세 화면에서 ‘삭제’ 버튼 클릭 | postId, githubId | postId 유효 & githubId(작성자) | postId 없음 | UC 3.3.1 | H | P |
| 3.3.2 |  |  | 삭제 확인 팝업 제공 | 팝업 '예' 선택 | 삭제됨 | '아니오' 선택 → 취소 | UC 3.3.2 | H | P |
| 3.3.3 |  |  | 타인은 삭제 버튼 미노출 | githubId | githubId(작성자 아님) |  | UC 3.3.3 | H | P |
| 3.4 | 게시글 목록 조회 | 전체 게시글 목록 조회 | 페이징 및 정렬 기능을 포함한 게시글 목록 제공 | 게시글 목록 버튼 |  |  | UC 3.4 | H | P |
| 3.4.1 |  |  | 번호·제목·작성자·작성일·조회수·댓글 수 표시 | page=1, size=10 | page≥1, size≥1 | page≤0, size≤0 | UC 3.4.1 | H | P |
| 3.4.2 |  |  | 최신순·조회순 정렬 | sortType | createdAt 또는 viewCount | invalid sortType | UC 3.4.2 | H | P |
| 3.4.3 |  |  | 게시글 클릭 시 상세 화면 이동 | postId | postId 존재 | 삭제됨/없음 | UC 3.4.3 | H | P |
| 3.5 | 게시글 상세 조회 | 게시글 상세 정보 표시 | 내용 + 메타 정보 + 댓글 목록 표시 | postId | postId 유효 | postId 없음 | UC 3.5 | H | P |
| 3.5.1 |  |  | 제목·내용·작성일·조회수·댓글 목록 표시 | postId | 정상 표시 | 삭제됨 | UC 3.5.1 | H | P |
| 3.5.2 |  |  | 상세 진입 시 조회수 증가 (작성자 제외) | postId, githubId | githubId ≠ 작성자 → viewCount 증가 | githubId = 작성자 → 증가 없음 | UC 3.5.2 | H | P |
| 3.6 | 댓글 작성 | 댓글 등록 | 특정 게시글에 댓글 추가 | content, postId, githubId | content≥1자, postId 존재 | content="", postId 없음 | UC 3.6 | H | P |
| 3.6.1 |  |  | 댓글 등록 시 DB 저장 | 등록 버튼 | content≥1자 | content="" | UC 3.6.1 | H | P |
| 3.6.2 |  |  | 댓글 목록 최신순으로 표시 | 댓글 목록 화면 |  |  | UC 3.6.2 | H | P |
| 3.6.3 |  |  | 댓글 정보 표시 | 화면 표시 |  |  | UC 3.6.3 | H | P |
| 3.7 | 댓글 삭제 | Soft Delete | 본인 댓글만 삭제 가능 | 삭제 버튼, commentId, githubId | commentId 유효 | commentId 없음 | UC 3.7 | H | P |
| 3.7.1 |  |  | 삭제 버튼 클릭 시 삭제 요청 전송 | 삭제 버튼 | 정상 삭제 | commentId 없음 | UC 3.7.1 | H | P |
| 3.7.2 |  |  | 타인 댓글은 삭제 버튼 미표시 | githubId | githubId ≠ 작성자 |  | UC 3.7.2 | H | P |
| 3.7.3 |  |  | 삭제 확인 팝업 제공 | 팝업 ‘예’ 선택 | 삭제됨 | ‘아니오’ 선택 → 취소 | UC 3.7.3 | H | P |
| 4 | **오픈소스 이슈** | | | | | | | | |
